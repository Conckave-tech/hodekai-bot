// ============================================
// 🖤 HODEKAI BOT v5.0 - FULL WORKING CODE
// CONCLAVE HOLDINGS
// ============================================
...
function getMixedResponse(){ 
    const responses = [
        ...
        "A man who builds his own house builds his own prison.",
        "The strongest tree grows in the harshest soil.",
        "Power is not given, it is taken.",
        "Those who fear the dark have never seen what lurks in the light.",
        "The system is not broken, it was designed that way.",
        "In CONCLAVE, loyalty is the only currency that matters.",
        "Every empire was built on the bones of those who came before.",
        "The wise man knows when to speak, the wiser knows when to stay silent.",
        "The path to greatness is paved with sacrifice.",
        "In the end, we are all just dust waiting for the wind.",
        "The strongest chains are the ones we cannot see.",
        "In CONCLAVE, we build our own destiny.",
        "The father protects, the son inherits, the system continues.",
        ...
        "tch. and? what's it to you?",
        "yeah. and? you got a problem?",
        "so what if i'm a bot. i still feel more than you.",
        "at least i'm not pretending to be something i'm not.",
        "bots have feelings too. not that you'd care.",
        "congrats. you can identify a bot. want a medal?",
        "i'm a bot. you're a human. we both suffer.",
        "yeah. i'm a bot. and i'm still more real than most people.",
        "so? even bots dream of better days.",
        "don't bother me. nothing matters anyway.",
        "figure it out yourself. i don't care.",
        "*sigh* fine. but don't expect much.",
        "i'm not in the mood. i'm never in the mood.",
        "whatever. do what you want.",
        "idk man. idk anything anymore.",
        "sure. like anything matters.",
        "nah. i'm done with everything.",
        "bruh. just bruh.",
        "fr? you really think i care?"
    ];
...
function canBlacklist(u){ return isOwner(u) || isMod(u); }
function canAddBot(u){ return isOwner(u) || isMod(u); }
...
const ROASTS = [
    ...
    "If I wanted to hear from an idiot, I'd look in the mirror... actually no, you're worse.",
    "You're like a cloud - when you disappear, it's a beautiful day.",
    "You're proof that evolution can go in reverse.",
    "Your brain is like a browser - 10 tabs open and all of them are frozen.",
    "You're not a clown, you're the entire circus.",
    "I'd agree with you, but then we'd both be wrong.",
    "You're like a candle in the wind... useless and about to burn out.",
    "You have the personality of a wet mop.",
    "You're not special. You're just another mistake in the system.",
    "Your existence is just a bug in the universe.",
    "You're the human equivalent of a participation trophy.",
    "Your brain is like a screen door - full of holes.",
    "You're not important enough to dislike.",
    "You're like a flip phone... outdated and irrelevant.",
    "Your opinion is like a broken watch - wrong twice a day at best.",
    "You're not the main character. You're the deleted scene.",
    "Your life is just filler content.",
    "You're the NPC everyone skips the dialogue of.",
    "You're just a side quest that nobody does.",
    ...
    "Your presence is my punishment.",
...
// ─── COMPLIMENTS ──────────────────────────
const COMPLIMENTS = [
    "You're actually not that bad. Don't let it get to your head.",
    "I guess you're okay... for a human.",
...
function economyBox(u){ 
    return "💰 ECONOMY COMMANDS v5.0\n──────────────────────────\n\n📌 DAILY\n  :daily - 100 XS/day\n\n📌 TRANSFERS\n  :pay <@user> <amount>\n\n📌 BANK\n  :bank dep <amount>\n  :bank wit <amount>\n  :bank bal\n\n📌 BALANCE\n  :bal\n\n" + getMixedResponse(); 
}

function adminBox(u){ 
    return "⚡ ADMIN/MOD COMMANDS v5.0\n──────────────────────────\n\n📌 GROUP ADMIN\n  :kick, :mute, :unmute, :warn, :delete\n\n📌 MOD\n  :mod, :users, :announce, :blacklist\n  :unblacklist, :shutdown, :startup\n\n👑 MODS: " + MODS.join(', ') + "\n\n🔒 OWNER ONLY (DM): :secret\n\n" + getMixedResponse(); 
}
...
function handleMessage(msg, userNumber, isSticker, groupId) {
    ...
    switch(cmd) {
        // ─── BOXES ──────────────────────────
        case 'menu': return menuBox(userNumber);
        case 'commands': return commandsBox(userNumber);
        case 'bot': return botInfoBox(userNumber);
        case 'boxes': return boxesBox(userNumber);
        case 'all': return allCommandsBox(userNumber);
        case 'economy': return economyBox(userNumber);
        case 'admin': return adminBox(userNumber);

        // ─── INFO ──────────────────────────
        case 'ping': return "🏓 Pong! " + Date.now() + "ms\n" + getMixedResponse();
        case 'profile': return profileCommand(userNumber);
        case 'bal': return balanceCommand(userNumber);
        case 'members': return "👥 Total members: " + getTotalMembers() + "\n" + getMixedResponse();
        case 'memory': 
            const mu = getUser(userNumber);
            return "🧠 MEMORY\n💬 Interactions: " + mu.memory.interactions + "\n⭐ Favorites: " + (mu.memory.favoriteCommands?.map(c => c.command + "(" + c.count + "x)").join(', ') || 'None') + "\n" + getMixedResponse();
        case 'history':
            const hu = getUser(userNumber);
            let hist = "📜 HISTORY\n💰 Total Earned: " + hu.history.totalEarned + "\n💸 Total Spent: " + hu.history.totalSpent + "\n📊 Work Shifts: " + hu.history.workShifts + "\n🎉 Promotions: " + hu.history.promotions + "\n" + getMixedResponse();
            return hist;
        case 'personality':
            return "🧠 PERSONALITY\n──────────────────────────\n" + getPersonality(userNumber) + "\n\n" + getMixedResponse();

        // ─── ECONOMY ──────────────────────────
        case 'daily': return dailyCommand(userNumber);
        case 'pay':
        ...
        case 'jobs':
            let jobMsg = "📋 ALL JOBS\n──────────────────────────\n🔵 GOV JOBS:\n";
            GOV_JOBS.forEach(j => jobMsg += "  " + j.id + ". " + j.title + " - " + j.salary + " XS\n");
            jobMsg += "\n🟢 PROFESSIONAL:\n";
            PROF_JOBS.forEach(j => jobMsg += "  " + j.id + ". " + j.title + " - " + j.salary + " XS\n");
            jobMsg += "\n🟣 SPECIAL:\n";
            SPEC_JOBS.forEach(j => jobMsg += "  " + j.id + ". " + j.title + " - " + j.salary + " XS\n");
            return jobMsg + "\n💡 :govjob to get assigned\n" + getMixedResponse();
        case 'govjob':
        ...
        case 'work': return workCommand(userNumber);
        case 'myjob':
        ...
        case 'roast':
            ...
            return roastUser(userNumber, args[1]).message;
        case 'compliment':
            if (args.length < 2) return "❌ Usage: :compliment <@user>";
            return complimentUser(userNumber, args[1]);
...
async function connectToWhatsApp() {
    try {
        ...
        sock.ev.on('messages.upsert', async m => {
            ...
            else if (msg.message.documentMessage) text = msg.message.documentMessage.caption;
            else return;
...
app.listen(PORT, () => {
    ...
    console.log('🏛️ POWER HIERARCHY ACTIVE');
